...
from generate_mock_data import build_prep_logs, build_workflow_reference
...
def test_valid_row_gets_parsed_and_duration_computed():
    ...
    assert cleaned.loc[0, "prep_time_valid"] is True or cleaned.loc[0, "prep_time_valid"] == True
...
def test_missing_prep_end_is_flagged_not_dropped():
    ...
    assert cleaned.loc[0, "prep_time_valid"] == False
...
def test_zero_duration_is_valid():
    ...
    assert cleaned.loc[0, "prep_time_valid"] == True
...
def test_duplicate_order_id_is_flagged_not_dropped():
    ...
    assert cleaned.loc[0, "order_id_duplicate"] == False
    assert cleaned.loc[1, "order_id_duplicate"] == True
    assert cleaned.loc[2, "order_id_duplicate"] == False
...
def test_clean_prep_logs_against_real_mock_data():
    ...
    assert (~cleaned["prep_time_valid"]).sum() > 0
    assert cleaned["prep_time_valid"].sum() > 0
    assert cleaned["order_id_duplicate"].sum() > 0
