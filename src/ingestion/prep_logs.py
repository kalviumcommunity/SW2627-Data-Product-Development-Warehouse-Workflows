...
def read_prep_logs(path: Path) -> pd.DataFrame:
    ...
    df = pd.read_csv(path, dtype=str)  # dtype=str: don't guess types yet,
    # that's cleaning's job (PRD 6.2); reading as str avoids pandas
    # silently coercing e.g. a malformed timestamp column into NaN/object
    # in an inconsistent way before we've had a chance to inspect it.
